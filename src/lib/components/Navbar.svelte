...
<header class="fixed top-0 left-0 right-0 z-50">
  <nav
    ...
    class:bg-opacity-0={!hasScrolled}
    style="transition:0.5s;"
    class="px-4 py-3 relative bg-black"
  >
    <!-- Desktop Navbar -->
    <div class="flex justify-between items-center">
      ...
      <div class="corner">
        <a href="/" class="flex items-center justify-center gap-3">
          <img style="transform:scale(1) pt-1" src={prowlIcon} alt="Prowl" />
          <span class="text-lg font-semibold"> Prowl</span>
        </a>
      </div>

      <!-- Desktop Links -->
      <div
        class="hidden md:flex items-center justify-center space-x-6 mx-auto pr-[0.15rem] pt-2 uppercase fixed top-0 left-0 right-0 h-14"
      >
        <a
          href="/"
        ...
        <a
          href="/contributing"
          class="hover:text-gray-400"
          class:active-link={$page.url.pathname.startsWith("/contributing")}
          >Contributing</a
        >
      </div>

      <!-- Social Links -->
      <div class="corner ml-auto px-3 flex gap-4">
    ...
    {#if isMobileMenuOpen}
      <div class="mobile-menu md:hidden px-4 pb-2">
        ...
        <a href="/contributing" class="block py-2 hover:text-gray-400">
          Contributing
        </a>
      </div>
    {/if}
